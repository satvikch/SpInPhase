function [error_criteria,Error_labels]=function_Errors_NoNormalization(X,Xest,Y,displabels)
displabels=1;
%%%%%%% CORRECTION ON MEAN VALUE %%%%%%%%%%%%%%%

    %%% modified by J  Bioucas Dias %%%%%%%%%%%%%%%%%%%%
    % reason: phase Y is is unwapped and thus defined up
    % to a constant
    mmean=mean(Y(:)-X(:));
    Y=Y-mmean;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% MED     Median  Absolute Difference
%
%
...
%     end
% end;
SNR=10*log10(sum(abs(X(:).*X(:)))/(sum(abs(Xest(:)-X(:)).^2))+.000000000001);
SNR1=sum(abs(X(:).*X(:)))/(sum(abs((Xest(:)-X(:)+eps)))+.000000000001);
%keyboard
mae_noisy=mean(abs(Xest(:)-X(:)));
mse_noisy=mean((Xest(:)-X(:)).^2);
rmse_noisy=sqrt(mse_noisy);
psnr_noisy=20*log10((maxl2+0.00000001)/(rmse_noisy));
%keyboard
maxdif=max(abs(Xest(:)-X(:)));
med=median(abs(Xest(:)-X(:)));
error_criteria=[SNR ; psnr_noisy; mse_noisy; rmse_noisy; mae_noisy; maxdif; med];
...
Error_verbose= ['SNR (dB) Signal-to-Noise Ratio                           : ';
                'PSNR Peak Signal-to-Noise Ratio                          : ';
                'MSE  Mean Squared Error          (l-2 norm squared)      : ';
                'RMSE Root of Mean Squared Error  (l-2 norm)              : ';
                'MAE  Mean Absolute Error         (l-1 norm)              : ';
                'MAX  Maximum Absolute Difference (l-infinity norm)       : '];
if ISNR_on==1
    ISNR = 10*log10(sum(abs(X(:)-Y(:)+eps).^2)/sum(abs(Xest(:)-X(:)).^2));
    error_criteria=[ISNR; error_criteria];
    Error_labels = ['ISNR(dB): '; Error_labels];
    Error_verbose= ['ISNR(dB) Improvement in Signal-to-Noise Ratio            : '; Error_verbose];
end

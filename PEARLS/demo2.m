%%  demo 2: reproduces PEARLS results shown in Figure 3 of paper
%
...
%  ---------------------------------------------------------------------
% 
% Original phase: causal AR model  
%  
...
fc = 0.05; % define cut frequency
mask = zeros(M,N);
mask(round(M/2-fc*M/2):round(M/2+fc*M/2), ...
     round(N/2-fc*N/2):round(N/2+fc*N/2)) = 1;;
 
varphi =  240* real(ifft2(fftshift(fftshift(fft2(varphi)).*mask)));
...
title('Window size at each pixel', 'FontSize', 32)
...
phi_hat_puma = puma_ho(phi_hat,2, 'potential',potential);  
...
phi_puma = puma_ho(phi,2, 'potential',potential);
